import React from "react"

function ConnectionBox() {
	return (
		<svg width="1em" height="1em" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path
				d="M43 4H5C4.44772 4 4 4.48842 4 5.09091V14.9091C4 15.5116 4.44772 16 5 16H43C43.5523 16 44 15.5116 44 14.9091V5.09091C44 4.48842 43.5523 4 43 4Z"
				fill="#2F88FF" stroke="#212121" stroke-width="3" stroke-linejoin="round"/>
			<path
				d="M43 32H5C4.44772 32 4 32.4884 4 33.0909V42.9091C4 43.5116 4.44772 44 5 44H43C43.5523 44 44 43.5116 44 42.9091V33.0909C44 32.4884 43.5523 32 43 32Z"
				fill="#2F88FF" stroke="#212121" stroke-width="3" stroke-linejoin="round"/>
			<path d="M14 16V24.0083L34 24.0172V32" stroke="#212121" stroke-width="3" stroke-linecap="square"
			      stroke-linejoin="round"/>
			<path d="M18 38H30" stroke="#FFF" stroke-width="3" stroke-linecap="square" stroke-linejoin="round"/>
			<path d="M18 10H30" stroke="#FFF" stroke-width="3" stroke-linecap="square" stroke-linejoin="round"/>
		</svg>
	)
}

export default ConnectionBox
